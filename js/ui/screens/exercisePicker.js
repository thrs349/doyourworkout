...
// v2.6.0: 기존 그룹 헤더(section-label) + 유형별 그룹 렌더링을 제거했었습니다. "운동 유형 버튼을 선택하면
// 해당 유형만 표시 → 표시되는 목록은 각 유형 내부 가나다순"으로 동작했고, 선택하지 않은 기본 상태에서도
// 유형순 정렬 자체는 항상 적용돼 있었습니다.
// v3.1.1: exerciseManage.js와 동일하게 기본 진입 시 가나다순만 사용하고, "운동 유형"/"운동 부위" 버튼을
// 선택했을 때만 해당 기준으로 그룹 정렬하도록 변경합니다(두 화면 정렬 규칙 완전히 동일하게 유지).
...
export function renderExercisePicker(root, params) {
  ...
  // "운동 부위" 선택 시 부위별 그룹으로 정렬 기준을 전환합니다. (exerciseManage.js와 동일 로직)
