...
const CACHE_NAME = "doyourworkout-v2.6.2"; // v2.6.2: 실기기(Galaxy S25) 테스트 반영 UI 버그 수정 릴리즈. APP_VERSION과
// 동일한 라벨로 캐시를 새로 무효화합니다(activate 핸들러가 이 값과 다른 기존 캐시 키를 전부 삭제하므로, 기존 설치
// 사용자도 다음 방문 시 자동으로 새 캐시로 전환됩니다). 캐시 전략 로직 자체는 무변경.
const CORE_ASSETS = [
  "./",
  "./index.html",
  "./manifest.json",
